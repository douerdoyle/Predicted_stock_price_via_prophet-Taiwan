...
class Stocker:
    ...
    # Initialization requires a ticker symbol
    def __init__(self, stock_code, stock, png_save_path=None, training_years=1, training_months=6):
    ...
    def changepoint_prior_analysis(
        ...
        for i, prior in enumerate(changepoint_priors):
            ...
            model.fit(train)
            future = model.make_future_dataframe(periods=180, freq="day")
    ...
    def create_prophet_model(self, days=1, resample=False):
        ...
        days+=(datetime.now().date()-self.max_date).days

        tmp_dt = self.max_date+timedelta(days=days)
        while tmp_dt.weekday() >= 5:
        ...
        if days > 0:
            ...
            lastest_price = list(stock_history[stock_history['Date'] == max(stock_history['Date'])]['Close'])[0]
            predicted_date = future.loc[future.index[-1], "ds"]
            sss = "Predicted Price on {} = {}, Confidence Interval {} ~ {}".format(
                    predicted_date.strftime('%Y-%m-%d'),
                    float("{:.2f}".format(future.loc[future.index[-1], "yhat"])),
                    round(list(future["yhat_lower"])[-1], 2),
                    round(list(future["yhat_upper"])[-1], 2)
                )
            min_date = min(stock_history['Date']).strftime('%Y-%m-%d')
            max_date = max(stock_history['Date']).strftime('%Y-%m-%d')
            title = f"{self.symbol}, Data time span: {min_date} ~ {max_date}, Lastest price: {lastest_price}\n{sss}"
        else:
        ...
        plt.title(title)
        if self.png_save_path:
            plt.savefig(self.png_save_path, dpi=400)
        # plt.show()

        return model, future, predicted_date
